...
class DialogPhase1(QWidget):
    ...
    def createLayout(self):
        ...
        row += 1
        self.nextPhase2 = QPushButton("Proceed with all combos to next phase")
        self.nextPhase2.clicked.connect(self.runPhaseTwoB)
        self.nextPhase2.setVisible(False)
        self.layout.addWidget(self.nextPhase2, row, 0, 1, 7)
        self.setLayout(self.layout)
        row += 1
        bottomlabel1 = QLabel("- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -")
    ...
    def afterweeksvisible(self,inp):
        ...
        self.nextPhase2.setVisible(weeksvis[0])
    ...
    def runPhaseTwo(self):
        ...
        dialog.show()
    def runPhaseTwoB(self):
        finals = []
        for shiftSeries in self.shiftseries:
            shiftSerie = []
            for s in shiftSeries.split(" "):
                shiftSerie.append(int(s))
            finals.append([shiftSerie[i * 7:(i + 1) * 7] for i in range((len(shiftSerie) + 6) // 7 )])
        shifts0 = [self.shiftLabel1, self.shiftLabel2, self.shiftLabel3]
        shifts = []
        for n in range(self.shifttype):
            shifts.append(shifts0[n])
        dialog = findAcceptableCombosGUI.DialogPhase2(self.shifttype,shifts,finals,self.shiftlengths,self.weeklyresting)
        self.phase2dialogs.append(dialog)
        dialog.show()
